...
def generate_scgoss_references():
    tests = gather_metadata("SCSTG/tests", "id")
    demos = gather_metadata("SCSTG/demos", "id")
...
def on_pre_build(config):
    config.cross_references = generate_scgoss_references()
        ...
        if weakness_id in cross_references["weaknesses"]:
            ...
            if tests:
                ...
                for test in tests:
                    ...
                    tests_section += f"[{get_platform_icon(test['platform'])} {test['id']}: {test['title']}]({relPath}){{: .SCS-test-button}} "
                markdown += f"\n\n{tests_section}"

    if "SCSTG-TEST-" in path:
        test_id = meta.get('id')
        if test_id in cross_references["tests"]:
            ...
            if demos:
                ...
                for demo in demos:
                    ...
                    demos_section += f"[{get_platform_icon(demo['platform'])} {demo['id']}: {demo['title']}]({relPath}){{: .SCS-demo-button}} "
